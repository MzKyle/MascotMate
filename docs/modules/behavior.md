...
`Main.gd` 会把 `CompanionMemory.gd` 的记忆快照和当前皮肤的 `personality` 放入行为上下文。当前阶段这些上下文只用于表达库选择文案：例如根据熟悉度、偏好互动、最近说过的句子和 `tone` 避免重复或选择更贴合的气泡。`BehaviorBrain.gd` 不会因为记忆或人格改变行为权重、冷却和打扰等级。
...
- v2 记忆快照和皮肤人格，用于自动事件记录后的记忆刷新和表达上下文
